...
var require_package = __commonJS({
  "package.json"(exports, module) {
    module.exports = {
      ...
      homepage: "https://github.com/any-u/ng#readme",
      bugs: {
        url: "https://github.com/any-u/ng/issues"
      },
      repository: {
        type: "git",
        url: "git+https://github.com/any-u/ng.git"
      },
      files: [
        "dist",
        "bin"
...
var camelcase = (input) => {
  return input.replace(/([a-z])-([a-z])/g, (_3, p1, p2) => {
...
function get(path3) {
  const content = fs.readFileSync(path3);
...
function generate(options) {
  ...
  while (options.length) {
    const path3 = options.shift();
    const value = options.shift();
    _.set(res, path3, value);
  }
  return res;
}
var config_default = {
...
// src/options.ts
init_esm_shims();
function excludeOptions(options, args) {
  const res = {};
...
function pickOptions(options, args) {
  const res = {};
  for (let key in options) {
    if (args.includes(key)) {
      res[key] = options[key];
...
function isFile(path3) {
  return fs2.existsSync(path3) && fs2.statSync(path3).isFile();
}
function getFiles(src, fileList = []) {
  ...
  files.forEach((file) => {
    const filePath = path2.join(src, file);
    if (isFile(filePath)) {
...
async function generate2(src, config) {
  const raw = fs3.readFileSync(src, "utf-8");
  return Handlebars.compile(raw)(config);
}
function combile(raw, config) {
  return Handlebars.compile(raw)(config);
}
async function writeTemplate(src, dest, config) {
  const content = await generate2(src, config);
  await fs3.ensureFileSync(dest);
  await fs3.writeFile(dest, content);
}

// src/download/local.ts
async function downloadLocal(name, config, args) {
  const { path: path3, alias, dir, options } = config;
  if (!path3) {
    console.log(picocolors.red(`[Ng] Failed to found boilerplate for ${picocolors.yellow(alias)}, please check the boilerplate path.`));
    return;
  }
  if (!fs4.existsSync(path3)) {
    console.log(picocolors.red(`[Ng] no such file or directory, open ${path3}`));
    return;
  }
  const root = dir ? `${process.cwd()}/${dir}` : process.cwd();
  const opts = pickOptions(args, options.map((option) => option.value));
  if (isFile(path3)) {
    const base = basename(path3);
    const ext = extname(path3);
    const dest = name ? `${root}/${name}${ext}` : `${root}/${base}`;
    const filename = basename(path3, ext);
    if (!opts.name) {
      opts.name = name ? name : filename;
    }
    await writeTemplate(path3, dest, opts);
  } else {
    const dir2 = name ? name : alias;
    if (!opts.name) {
      opts.name = dir2;
    }
    const files = getFiles(path3);
    const response = await Promise.allSettled(files.map((file) => {
      const basename3 = cull(file, path3);
      return writeTemplate(file, `${root}/${dir2}/${basename3}`, opts);
    }));
    for (let res of response) {
      if (res.status === "rejected") {
        res.reason && console.log(picocolors.red(`[Ng] Failed to write <Template>, because of ${res.reason}
`));
      }
    }
  }
...
function processPath(path3, alias, name, options, isFile3 = true) {
  const pathList = path3.split("/");
  const origin = basename2(path3);
  if (isFile3) {
    const ext = extname2(path3);
    const base = basename2(path3, ext);
    const filename = options.name ? options.name + ext : name ? name + ext : origin;
    if (!options.name) {
  ...
  const res = pathList.map((path4) => path4 === alias ? dirname : path4).join("/");
  console.log();
  console.log(res);
  return res;
}
async function downloadRemote(name, config, args) {
  const { alias, url, token, dir, options } = config;
  if (!url) {
    console.log(picocolors2.red(`[Ng] Failed to found boilerplate for ${picocolors2.yellow(name)}, please check the boilerplate url.`));
    return;
  }
  if (!token) {
    console.log(picocolors2.red(`[Ng] Failed to found token, boilerplate token must be provided.`));
    return;
  }
  const root = dir ? `${process.cwd()}/${dir}` : process.cwd();
  const opts = pickOptions(args, options.map((option) => option.value));
  try {
    ...
    await download(options2, (path3) => processPath(path3, alias, name, opts, isFile2(url)), (content) => combile(content, opts));
  } catch (error) {
    console.log(picocolors2.red(`[Ng] Failed to download boilerplate, because of ${error.message}`));
  }
}

// src/download/index.ts
async function download2(name, config, args) {
...
// src/inquirer.ts
init_esm_shims();
var import_yaml2 = __toESM(require_yaml());
import inquirer from "inquirer";
import picocolors3 from "picocolors";
function validate(config) {
  const { alias, origin, options } = config;
  if (!alias) {
    console.log(picocolors3.red(`[Ng] Failed to found alias, boilerplate alias must be defined.`));
    return false;
  }
  if (!origin) {
    console.log(picocolors3.red(`[Ng] Failed to found origin, boilerplate origin must be defined.`));
    return false;
  }
  if (origin === "local" && !config.path) {
    console.log(picocolors3.red(`[Ng] Failed to found path, boilerplate path must be defined when the origin is local`));
    return false;
  }
  if (origin === "remote" && !config.url) {
    console.log(picocolors3.red(`[Ng] Failed to found url, boilerplate url must be defined when the origin is remote`));
    return false;
  }
  if (options.length) {
    for (let option of options) {
      if (!option.value) {
        console.log(picocolors3.red(`[Ng] Failed to found value for ${option.label} option`));
        return false;
      }
    }
  }
...
async function addBoilerplate() {
  try {
    ...
    const config = import_yaml2.default.parse(answer.config);
    if (validate(config)) {
      return config;
    }
  } catch (error) {
...
async function deleteBoilerplate(choices) {
  try {
    const answer = await inquirer.prompt([
      {
        type: "checkbox",
...
async function updateBoilerplate(config) {
  try {
    const answer = await inquirer.prompt([
      {
        type: "list",
      ...
      },
      {
        ...
        default: (answer2) => {
          return import_yaml2.default.stringify(config[answer2.alias]);
        }
      }
    ]);
    const res = import_yaml2.default.parse(answer.config);
    if (validate(res)) {
...
async function useInquirer(config) {
  try {
    ...
    if (method === 1) {
      const res = __spreadValues({}, config);
      ...
      const choices = Object.keys(config).map((alias2) => ({
        value: alias2,
        name: alias2
      }));
...
// src/command.ts
function displayCommand(configuration) {
  const p = new Table({
    title: "Boilerplate",
    columns: [
      {
        name: "alias",
        title: picocolors4.cyan("alias"),
        alignment: "center"
      },
      {
        name: "origin",
        title: picocolors4.cyan("origin"),
        alignment: "center"
      },
      {
        name: "address",
        title: picocolors4.cyan("path/url"),
        alignment: "center"
      },
      {
        name: "options",
        title: picocolors4.cyan("options"),
        alignment: "center"
      }
    ]
  });
...
async function downloadCommand(name, alias, configuration, options) {
  ...
  await download2(name, configuration[alias], args);
}
